...
if (isset($_POST['submit'])) {
    ...
    $query = "SELECT * FROM student_att WHERE class_id= '$class_id' AND att_date = '$date'";
    $result = $conn->query($query);

    if (!$result->num_rows > 0) {
        $query = "SELECT * FROM student_course WHERE class_id= '$class_id'";
        $result = $conn->query($query);
        while($row = mysqli_fetch_array($result)){
			...
			{
				$query1 = "INSERT INTO student_att(class_id, student_id, att_status, att_date, student_name, course_code,course_name,section)
                VALUES ('$class_id', '$stu_id', '1','$date','$student_name','$course_code', '$course_name', '$section')";
			}
			else
			{
				$query1 = "INSERT INTO student_att(class_id, student_id, att_status, att_date)
                VALUES ('$class_id', '$stu_id', '0','$date')";
			}
            mysqli_query($conn,$query1);
        }
        echo "<script> alert('Attdence Successfully Taken.');
        </script>";


    }else{
        echo "<script> alert('Attdence Already Taken.');</script>";
    }
                                    ...
                                    <form action="" method="POST">
                                        <div class="row justify-content-center" style="padding-bottom:20px;">
                                            <div class="col-md-4">
                                                <div class="form-group" style="padding:10px">
                                                    <label style="padding-bottom:10px; font-weight:700">Attendence
                                                        Date</label>
                                        ...
                                        <table class="table">
                                            ...
                                            <tbody>
                                                ...
                                                <?php 
                                                $sql = "SELECT * FROM student_course WHERE class_id='$class_id' AND sem_id=(SELECT sem_id FROM semester ORDER BY sem_id DESC LIMIT 1)";
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <input type=checkbox name="<?php echo $student_id?>" onclick="getatt(this.checked);"/>   Present
                                                    </td>
                                                </tr>
                                                <?php
                                                            $i++;
                                        ...
                                        <div class="d-flex justify-content-end" style="padding-top:10px;">
                                            <button type="submit" name="submit" class="btn btn-success"
                                                style="margin-right:10px;">Submit Attendence</button>
